...
def object_point_cloud_b(object, points_local):
    """
    points_local: (num_envs, num_points, 3)
    object: RigidObject
    """
    num_envs, num_points, _ = points_local.shape

    object_pos_w = object.data.body_link_pose_w[:, 0, 0:3]        # (num_envs, 3)
    object_quat_w = object.data.body_link_pose_w[:, 0, 3:]        # (num_envs, 4)

    # Расширяем до (num_envs, num_points, 3/4)
    object_pos_w_expanded = object_pos_w.unsqueeze(1).repeat(1, num_points, 1)
    object_quat_w_expanded = object_quat_w.unsqueeze(1).repeat(1, num_points, 1)

    # apply rotation + translation
    points_w = quat_apply(object_quat_w_expanded, points_local) + object_pos_w_expanded
    return points_w  # (num_envs, num_points, 3)
